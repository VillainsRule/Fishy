// Version 1.0.0.7
const version = '1.0.0';
const versionNotes = 'Dev Edit 7'
const internalVersion = '1.0.0.7';
...
console.log(chalk.yellowBright(`╰ If you encounter an error, open an issue: ${chalk.bold('https://github.com/VillainsRule/Fishy/issues')}\n`))

if (process.version.match(/^v(\d+\.\d+)/)[1] < 20) console.log(chalk.redBright('You are running a NodeJS version under v20. If you don\'t upgrade, you may get large lag spikes or ram overloads.'));

const config = process.env.config ? JSON.parse(process.env.config) : JSON.parse(fs.readFileSync('./config.json', 'utf-8'));
...
https.get('https://raw.githubusercontent.com/VillainsRule/Fishy/main/src/main.js', (res) => {
    let data = '';
...
async function start(token, channelId) {
    ...
    client.on('ready', async () => {
        ...
        if (config.autoDaily) {
            ...
            let remainingTime;
            if (now > gmt0) remainingTime = (new Date(gmt0).setUTCDate(new Date(gmt0).getUTCDate() + 1)) - now;
    ...
    client.on('messageCreate', async (message) => {
        ...
        if (message.embeds[0]?.description?.includes('boost ended')) queueCommands.push({
            command: 'shop boosts'
        });
        ...
        if (message.interaction.commandName === 'play' && message?.embeds[0]?.title?.includes('Inventory of ')) {
            ...
            if (Number(fishValue.slice(0, -2).slice(2)) > 15000) queueCommands.push({
                command: 'sell',
                args: ['all']
            });
        }
        ...
        if (message.embeds[0]?.footer?.text?.includes('You can disable these tips using the /set tips off command.') && config.disableTips) queueCommands.push({
            command: 'set',
            args: ['Tips', 'off']
        });
        ...
        if (message.embeds[0]?.description?.includes('You cast your rod and fail to reach the water.')) queueCommands.push({
            command: 'biome'
        });
        ...
        if (message.embeds[0]?.description?.includes('now have enough to afford a new rod') && config.autoBuy.rods) queueCommands.push({
            command: 'shop rods'
        });
